...
def knn_predict(train_data, test_vector, k_value):
    distances = train_data.apply(
        lambda row: np.sum(np.abs(test_vector - row.iloc[:512])), axis=1)
...
def evaluate_fold(train_data, test_data, k_value):
    ...
    for idx, row in test_data.iterrows():
        test_vector = row.iloc[:512].values
...
data_leaf = pd.read_csv("output_csv/hog.csv")
hu_moments_columns = data_leaf.columns[1:513]
